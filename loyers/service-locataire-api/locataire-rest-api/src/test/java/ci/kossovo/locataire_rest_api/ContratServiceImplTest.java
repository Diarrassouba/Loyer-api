...
public class ContratServiceImplTest {
    ...
    @Test
    @DisplayName("createContrat - Doit réussir si le bien est disponible et l'historique du locataire est bon")
    public void createContrat_shouldSucceed_whenAllChecksPass() {
        // Arrange
        String bienId = "apt-123";
        String locataireId = "loc-abc";
        ContratRequestDTO requestDTO = new ContratRequestDTO(LocalDate.now(), BigDecimal.valueOf(1000), locataireId,
                bienId, null);
        ...
        // (donc ok)
        when(disponibiliteRepository.findById(bienId)).thenReturn(Optional.of(new DisponibiliteBien(bienId)));
    ...
    public void createContrat_shouldFail_whenBienIsLoue() {
        // Arrange
        String bienId = "apt-123";
        ContratRequestDTO requestDTO = new ContratRequestDTO(LocalDate.now(), BigDecimal.valueOf(1000), "loc-abc",
                bienId, null);
        ...
        // Act & Assert
        assertThatThrownBy(() -> contratService.createContrat(requestDTO)).isInstanceOf(IllegalStateException.class)
    ...
    @Test
    @DisplayName("createContrat - Doit échouer si l'historique du locataire est mauvais (trop de retards)")
    public void createContrat_shouldFail_whenLocataireHistoryIsBad() {
        ...
        when(disponibiliteRepository.findById("apt-123")).thenReturn(Optional.of(new DisponibiliteBien("apt-123")));
    ...
    public void terminerContrat_shouldFail_whenContratAlreadyTerminated() {
        ...
        // Act & Assert
        assertThatThrownBy(() -> contratService.terminerContrat(contratId)).isInstanceOf(IllegalStateException.class)
    ...
    public void findContratById_shouldFail_whenContratNotFound() {
        ...
        // Act & Assert
        assertThatThrownBy(() -> contratService.findContratById(contratId)).isInstanceOf(EntityNotFoundException.class)
    ...
    public void createLocataire_shouldSucceed_whenValidRequest() {
        // Arrange
        LocataireRequestDTO requestDTO = new LocataireRequestDTO("Doe", "John", "john.doe@example.com", "0123456789");
    ...
    public void findLocataireById_shouldFail_whenLocataireNotFound() {
        ...
        assertThatThrownBy(() -> contratService.findLocataireById(locataireId))
                .isInstanceOf(EntityNotFoundException.class).hasMessageContaining("Locataire non trouvé");
        verifyNoInteractions(eventGateway);
    }
    ...
    public void updateLocataire_shouldFail_whenLocataireNotFound() {
        // Arrange
        String locataireId = "non-existent-locataire";
        LocataireRequestDTO requestDTO = new LocataireRequestDTO("Doe", "Jane", "jane.doe@example.com", "0987654321");
        when(locataireRepository.findById(locataireId)).thenReturn(Optional.empty());
        // Act & Assert
