...
public class TenancyControllerIntegrationTest {
  ...
  @Test
  @DisplayName(
      "POST /api/contrats - Échec : Doit retourner 400 et un message clair si le bien est loué")
  void createContrat_shouldReturn400_whenBienIsTaken() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message", is("Le bien immobilier " + bienId + " est déjà loué."))); // <--
    // Vérification
    // enrichie

    assertThat(contratRepository.count()).isZero();
    verify(eventGateway, never())
        .publish(any(ContratCreatedEvent.class)); // On vérifie qu'aucun événement
    // n'est
    // publié
  }
  ...
  @Test
  @DisplayName(
      "POST /api/contrats - Échec : Doit retourner 400 et un message clair si le locataire a un"
          + " mauvais score")
  void createContrat_shouldReturn400_whenLocataireHasBadScore() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.message", is("Le locataire a un score de comportement insuffisant."))); // <--
    // Vérification
    // enrichie
  }
  ...
  @Test
  @DisplayName(
      "POST /api/contrats - Échec : Doit retourner 400 pour des données de requête invalides (DTO"
          + " validation)")
  ...
  void createLocataire_shouldSucceed() throws Exception {
    // Arrange
    LocataireRequestDTO locataireRequest =
        new LocataireRequestDTO("Kone", "Moussa", "kone.moussa@email.com", "0123456789");
    // Act & Assert
    mockMvc
        .perform(
            post("/api/locataires")
  ...
  @Test
  @DisplayName(
      "POST /api/contrats/{id}/terminer - Succès : Doit terminer un contrat actif et retourner 200"
          + " OK")
  void terminerContrat_shouldSucceed_forActiveContrat() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(post("/api/contrats/{id}/terminer", savedContrat.getId()))
  ...
  @Test
  @DisplayName(
      "POST /api/contrats/{id}/terminer - Échec : Doit retourner 404 Not Found si le contrat"
          + " n'existe pas")
  void terminerContrat_shouldReturn404_whenContratNotFound() throws Exception {
    mockMvc
        .perform(post("/api/contrats/{id}/terminer", "contrat-inexistant"))
