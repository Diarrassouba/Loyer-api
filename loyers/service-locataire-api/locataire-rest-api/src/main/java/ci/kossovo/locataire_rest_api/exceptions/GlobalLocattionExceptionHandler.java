package ci.kossovo.locataire_rest_api.exceptions;
...
public class GlobalLocattionExceptionHandler {

    /**
     * Gère les violations de règles métier (ex: bien déjà loué, mauvais
     * historique). Ces erreurs sont des requêtes valides sur le plan de la syntaxe,
     * mais qui violent une règle métier. Un statut 400 Bad Request ou 409 Conflict
     * est approprié. Nous utilisons 400 pour la simplicité.
     */
    ...
    public ResponseEntity<ErrorResponseDTO> handleIllegalStateException(IllegalStateException ex, WebRequest request) {
        ...
        ErrorResponseDTO errorResponse = new ErrorResponseDTO(HttpStatus.BAD_REQUEST.value(), "Bad Request",
                ex.getMessage(), // Le message de l'exception contient la raison métier
                request.getDescription(false));
        return new ResponseEntity<>(errorResponse, HttpStatus.BAD_REQUEST);
    }

    /**
     * Gère les exceptions lorsqu'une entité (Contrat, Locataire) n'est pas trouvée.
     * Renvoie un statut 404 Not Found.
     */
    ...
    /**
     * Gère les arguments invalides (ex: associer un contrat à une maison ET un
     * appartement). Renvoie un statut 400 Bad Request.
     */
    ...
    public ResponseEntity<ErrorResponseDTO> handleGenericException(Exception ex, WebRequest request) {
        ...
        // logger.error("Une erreur interne est survenue dans le tenancy-service", ex);
