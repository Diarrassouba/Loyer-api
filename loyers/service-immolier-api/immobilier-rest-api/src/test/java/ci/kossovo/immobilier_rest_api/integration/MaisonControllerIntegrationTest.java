...
public class MaisonControllerIntegrationTest {
  ...
  void createMaison_shouldReturn201Created() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.lot", is("214 ilot 21 Attie")));
  ...
  void getMaisonById_shouldReturnMaisonWhenExists() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.id", is(maison.getId())))
        .andExpect(jsonPath("$.ville", is("Abidjan")));
  }
  ...
  @Test
  @DisplayName(
      "POST /api/maisons/{maisonId}/appartements - Doit créer un appartement et retourner 201"
          + " Created")
  void createAppartement_shouldReturn201Created() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/maisons/{maisonId}/appartements", maison.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(aptRequest)))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.id").isNotEmpty())
        .andExpect(jsonPath("$.reference", is("APT 3B")))
        .andExpect(jsonPath("$.maisonId", is(maison.getId())));
  ...
  @Test
  @DisplayName(
      "GET /api/maisons/{maisonId}/appartements - Doit retourner la liste des appartements")
  void getAppartementsByMaison_shouldReturnList() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/maisons/{maisonId}/appartements", maison.getId()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
        .andExpect(jsonPath("$[0].reference", is("APT 02")))
  ...
  void updateMaison_shouldReturn200Ok() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/maisons/{id}", maisonId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestDTO)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id", is(maisonId)))
        .andExpect(jsonPath("$.lot", is("Nouveau lot")));
  ...
  void updateMaison_shouldReturn404WhenNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            put("/api/maisons/{id}", nonExistentId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(requestDTO)))
  ...
  void deleteMaison_shouldReturn204NoContent() throws Exception {
    // ARRANGE
    Maison maison =
        maisonRepository.save(createMaisonEntity("180 ilot 9 Selmer", "Yopougon", "Abidjan"));
    String maisonId = maison.getId();
  ...
  @Test
  @DisplayName(
      "POST /api/maisons/{maisonId}/appartements - Doit retourner 404 Not Found si la maison"
          + " n'existe pas")
  void createAppartement_shouldReturn404WhenMaisonNotFound() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/maisons/{maisonId}/appartements", nonExistentMaisonId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(aptRequest)))
        .andExpect(status().isNotFound());
  ...
  @Test
  @DisplayName(
      "GET /api/maisons/{maisonId}/appartements - Doit retourner 404 Not Found si la maison"
          + " n'existe pas")
  void getAppartementsByMaison_shouldReturn404WhenMaisonNotFound() throws Exception {
    ...
    // ACT & ASSERT
    mockMvc
        .perform(get("/api/maisons/{maisonId}/appartements", nonExistentMaisonId))
        .andExpect(status().isNotFound());
  ...
  private void createAppartementApiCall(String maisonId, AppartementRequestDTO dto)
      throws Exception {
    mockMvc.perform(
        post("/api/maisons/{maisonId}/appartements", maisonId)
            .contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  @DisplayName(
      "POST /api/depenses - Doit créer une dépense pour une maison et retourner 201 Created")
  void createDepense_forMaison_shouldSucceed() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.maisonId", is(maison.getId())))
        .andExpect(jsonPath("$.appartementId").isEmpty());
  ...
  @Test
  @DisplayName(
      "POST /api/depenses - Doit créer une dépense pour un appartement et retourner 201 Created")
  ...
  void createDepense_shouldReturn400_whenNoAssociation() throws Exception {
    ...
    DepenseRequestDTO requestDTO =
        new DepenseRequestDTO(
            BigDecimal.TEN, "Dépense orpheline", LocalDate.now(), TypeDepense.DIVERS, null, null);
  ...
  @Test
  @DisplayName(
      "GET /api/maisons/{maisonId}/depenses - Doit retourner la liste des dépenses pour une maison")
  void getDepensesByMaison_shouldReturnDepenseList() throws Exception {
    ...
    // Act & Assert
    mockMvc
        .perform(get("/api/maisons/{maisonId}/depenses", maison1.getId()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$", hasSize(2)))
  ...
  private Depense createDepenseEntity(Maison maison, Appartement apt, String description) {
    ...
    if (maison != null) depense.setMaisonId(maison.getId());
    if (apt != null) depense.setAppartementId(apt.getId());
    return depense;
  }
}
